...
fn test_maximum_precision_changes() {
    ...
    let int_value = ManagedDecimal::<StaticApi, usize>::from_raw_units(
        BigUint::from(5u64), 
        0
    );
    let result2 = tester.rescale_half_up(&int_value, 27);
    assert_eq!(result2.into_raw_units(), &BigUint::from(5_000_000_000_000_000_000_000_000_000u128));
}
...
fn test_signed_edge_cases() {
    ...
    let _zero = ManagedDecimalSigned::<StaticApi, usize>::from_raw_units(
        BigInt::from(0i64),
        1
    );
