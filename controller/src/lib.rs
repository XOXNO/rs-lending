    ...
    + multiversx_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn supply(&self, is_vault: bool, e_mode_category: OptionalValue<u8>) {
        ...
        let (account_nonce, account_attributes) = self.get_or_create_position(
            &caller,
            first_asset_info.is_isolated_asset,
            is_vault,
    ...
    fn withdraw(&self, collaterals: MultiValueEncoded<EgldOrEsdtTokenPayment<Self::Api>>) {
        ...
        self.manage_nft_after_withdrawal(
            &account.amount,
            account.token_nonce,
    ...
    fn borrow(&self, borrowed_tokens: MultiValueEncoded<EgldOrEsdtTokenPayment<Self::Api>>) {
        ...
        let collaterals =
            self.sync_deposit_positions_interest(account.token_nonce, &mut storage_cache, false);
        ...
        let e_mode = self.get_e_mode_category(attributes.get_emode_id());
        ...
        for borrowed_token in borrowed_tokens {
            ...
            let asset_emode_config = self.get_token_e_mode_config(
                attributes.get_emode_id(),
                &borrowed_token.token_identifier,
            );

            self.ensure_e_mode_compatible_with_asset(&asset_config, attributes.get_emode_id());
    ...
    fn flash_loan(
        ...
        require!(asset_info.can_flashloan(), ERROR_FLASHLOAN_NOT_ENABLED);
        ...
        self.tx()
            ...
            .flash_loan(
                borrowed_token,
                ManagedDecimal::from_raw_units(amount, feed.asset_decimals as usize),
                contract_address,
                endpoint,
                arguments,
                asset_info.flashloan_fee.clone(),
                feed.price.clone(),
            )
    ...
    fn disable_vault(&self) {
        ...
        for mut bp in borrow_positions.values() {
            ...
            self.borrow_positions(account.token_nonce)
                .insert(bp.asset_id.clone(), bp);
        }

        for mut dp in deposit_positions.values() {
            let pool_address = storage_cache.get_cached_pool_address(&dp.asset_id);

            let feed = self.get_token_price(&dp.asset_id, &mut storage_cache);
            let old_amount = dp.principal_amount.clone();
            let last_value = self.vault_supplied_amount(&dp.asset_id).update(|am| {
                *am -= &old_amount;
                am.clone()
            });

            self.update_vault_supplied_amount_event(&dp.asset_id, last_value);
            ...
            self.deposit_positions(account.token_nonce)
                .insert(dp.asset_id.clone(), dp);
        }

        nft_attributes.is_vault_position = false;
    ...
    fn enable_vault(&self) {
        ...
        for mut bp in borrow_positions.values() {
            ...
            // Update storage with the latest position
            self.borrow_positions(account.token_nonce)
                .insert(bp.asset_id.clone(), bp.clone());
        }

        for mut dp in deposit_positions.values() {
            let asset_address = self.get_pool_address(&dp.asset_id);
            ...
            dp.is_vault_position = true;
            dp.principal_amount = total_amount_with_interest.clone();
        ...
        nft_attributes.is_vault_position = true;
    ...
    fn update_position_threshold(
        ...
        for account_nonce in account_nonces {
            ...
            let threshold = if nft_attributes.has_emode() {
                let e_mode_category = self.e_mode_category().get(&nft_attributes.get_emode_id());

                if is_ltv {
                    e_mode_category.unwrap().loan_to_value.clone()
                } else {
                    e_mode_category.unwrap().liquidation_threshold.clone()
                }
            } else {
                if is_ltv {
                    asset_config.loan_to_value.clone()
                } else {
            ...
            let current_threshold = if is_ltv {
                ...
                &dp.liquidation_threshold
            };

            if current_threshold != &threshold {
                if is_ltv {
                    dp.loan_to_value = threshold;
                } else {
                    dp.liquidation_threshold = threshold;
                }
