...
use common_structs::{
    ExchangeSource, FeeMoment, FeeToken, OracleProvider, OracleType, SwapStep, TokenInData,
};
    ...
    oracle::OracleModule + storage::Storage + math::MathsModule + common_math::SharedMathModule
{
    ...
    fn convert_token_from_to_arda(
        &self,
        to_token: &EgldOrEsdtTokenIdentifier,
        from_token: &EgldOrEsdtTokenIdentifier,
        from_amount: &BigUint,
        caller: &ManagedAddress,
        steps: Option<ArdaSwapArgs<Self::Api>>,
    ) -> EgldOrEsdtTokenPayment {
        if to_token == from_token {
            return EgldOrEsdtTokenPayment::new(to_token.clone(), 0, from_amount.clone());
        }
        self.swap_tokens_arda(to_token, from_token, from_amount, caller, steps)
    }
    ...
    fn swap_tokens_arda(
        ...
        from_amount: &BigUint,
        caller: &ManagedAddress,
        args: Option<ArdaSwapArgs<Self::Api>>,
    ) -> EgldOrEsdtTokenPayment {
        // Ensure steps and limits are provided
        require!(args.is_some(), "Args are required");
